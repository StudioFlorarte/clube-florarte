...
async function sendPending(form:FormData) {
 ...
 const {data:row,error}=await admin.from('subscriptions').select('invoice_id,email,product_id,delivery_locale,status,current_period_end,invite_sent_at').eq('invoice_id',invoice).single()
 ...
 try {
  ...
  const locale=row.delivery_locale || productLocales[row.product_id]
...
export default async function Invitations({searchParams}:{searchParams:{result?:string}}) {
 await requireAdmin()
 const {data:rows,error}=await createAdminClient().from('subscriptions').select('invoice_id,email,product_id,delivery_locale,invite_sent_at,current_period_end').eq('status','active').gt('current_period_end',new Date().toISOString()).order('updated_at',{ascending:false}).limit(100)
 ...
 <h2>{row.email}</h2><p>{(row.delivery_locale || productLocales[row.product_id])==='en'?'English':'Português'} · {row.invoice_id.startsWith('test:')?'Teste temporário':'Assinatura'} · {row.invoice_id}</p>
