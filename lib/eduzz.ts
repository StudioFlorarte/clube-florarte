...
export function parseInvoice(payload:any){
 ...
 return {eventId:payload.id,event:payload.event,invoice:String(d.id),email,product:String(item.productId),contract:d.contract?.id?String(d.contract.id):null,paidAt:new Date(d.paidAt).toISOString(),end:annualEnd(d.paidAt),name:String(d.student?.name||d.buyer?.name||'Membro').slice(0,100)}
}
